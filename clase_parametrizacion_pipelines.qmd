---
title: "Parametrización, Estructura de Carpetas y Pipelines"
subtitle: "De código enredado a pipelines profesionales"
...
Cuando trabajamos en proyectos de datos, es común empezar con código "rápido y sucio" para probar ideas. Sin embargo, este código presenta problemas:

- **Valores hardcodeados** dispersos por todo el script
- **Difícil mantenimiento**: cambiar un parámetro requiere editar múltiples líneas
- **No reutilizable**: cada nuevo caso requiere copiar y modificar el código
- **Propenso a errores**: fácil olvidar actualizar algún valor
- **Difícil de versionar**: credenciales y configuraciones mezcladas con lógica

**Objetivo de esta clase**: Transformar código desorganizado en pipelines profesionales, mantenibles y escalables.

---

# 2. Parametrización de Variables

## 2.1 De Código Enredado a Código Limpio

Veamos un ejemplo típico de código problemático:

```{python}
#| eval: false
#| code-line-numbers: true

# ❌ CÓDIGO MALO: valores hardcodeados y repetidos
import pandas as pd
...
# Calcular métricas con umbral hardcodeado
...
1. Rutas absolutas que solo funcionan en una máquina

2. Fechas repetidas (¿qué pasa si queremos analizar 2025?)

3. Umbral mágico (1000) sin contexto

4. Año repetido en strings
...
# ✅ CÓDIGO LIMPIO: parametrizado y mantenible
import pandas as pd
from pathlib import Path

# Definir parámetros en un solo lugar
YEAR = 2024
...
# Filtrar por fecha usando el parámetro
...
# Calcular métricas usando parámetro descriptivo
...
- ✅ Parámetros definidos una sola vez

- ✅ Nombres descriptivos en MAYÚSCULAS para constantes

- ✅ Rutas relativas usando `pathlib`. No es necesario usar `pathlib`, es una opción. Pero lo realmente importante es no hardcodear rutas absolutas, sino que usar rutas relativas al proyecto (no al archivo)

- ✅ Fácil modificar para otro año o umbral

---

## 2.2 Variables de Ambiente con .env

Para información sensible (credenciales, API keys) o configuraciones que cambian entre entornos (desarrollo, producción), usamos archivos `.env`.
...
3. Documentar en README cómo configurar el `.env`


4. A veces es bueno tener múltiples archivos `.env` para diferentes entornos:
...
Quizás algunos de ellos sí pueden comitearse, y otros no. Por defecto, los archivos `.env` no deberían comitearse nunca.

---

## 2.3 Argumentos de Línea de Comandos con argparse

`argparse` permite ejecutar scripts con diferentes parámetros sin modificar el código.
...
def procesar_ventas(year, umbral, verbose=False):
    """Procesa ventas de un año específico."""
    ...
    if verbose:
        ...
        print(f"✅ {len(clientes_importantes)} clientes > ${umbral}")
...
def main():
    ...
    parser.add_argument(
        ...
        type=float,
        default=1000,
        help='Umbral mínimo para cliente importante'
    )
...
# Ejecución básica (solo argumento requerido)
...
# 3. Parametrización de Queries

## 3.1 SQL con f-strings

Al construir queries dinámicamente, necesitamos insertar parámetros. Los f-strings son útiles pero debemos usarlos con cuidado.
...
**NUNCA** usar f-strings con input de usuarios sin validación:

```{python}
#| eval: false

# ❌ PELIGROSO: vulnerable a SQL injection
producto_usuario = input("¿Qué producto buscar? ")
query = f"SELECT * FROM ventas WHERE producto = '{producto_usuario}'"

# ✅ SEGURO: usar parámetros con placeholder
...
def construir_query_ventas(
    ...
    """Construye query dinámicamente según parámetros."""
...
## 3.2 Parametrización con Polars (Opcional)

Polars es una alternativa moderna y rápida a pandas. Ofrece formas elegantes de parametrizar queries.
...
- Más rápido que pandas en datasets grandes
...
- **Facilita encontrar archivos**: ubicación predecible
- **Separa concerns**: datos, código, resultados, documentación
- **Escalable**: crece con el proyecto sin volverse caótica
...
│   └── test_utils.py
│
├── .env                  # Variables de ambiente (NO subir a git)
...
🔗 [https://github.com/rodo-nunez/ejemplo_de_modularizacion_de_proyecto](https://github.com/rodo-nunez/ejemplo_de_modularizacion_de_proyecto)

**Nota importante:** No existe una estructura "perfecta". Adapta según:
...
├── cookiecutter.json          # Configuración
...
## 5.1 ¿Por Qué Modularizar?

Imagina un análisis típico:
...
# Un solo script gigante de 1000+ líneas
# Difícil de mantener, debuggear y reutilizar
import pandas as pd

# ... 100 líneas de extracción ...
# ... 200 líneas de limpieza ...
# ... 300 líneas de feature engineering ...
# ... 400 líneas de modelado ...
```
...
# Scripts separados, cada uno con responsabilidad única
scripts/
├── 01_extraer_datos.py       # Solo extracción
├── 02_limpiar_datos.py       # Solo limpieza
├── 03_crear_features.py      # Solo feature engineering
├── 04_entrenar_modelo.py     # Solo modelado
└── 05_generar_reporte.py     # Solo reporting
```

**Ventajas:**

- 🔍 **Debuggeable**: fácil identificar dónde falló

- 🔄 **Reutilizable**: usar solo los pasos necesarios

- 🤝 **Colaborativo**: diferentes personas trabajan en diferentes módulos

- ⚡ **Eficiente**: re-ejecutar solo pasos modificados

---

## 5.2 Orquestación con Bash
...
if [ $? -ne 0 ]; then
    echo "❌ Error en extracción. Pipeline detenido."
    exit 1
fi
...
echo ""
echo "🧹 PASO 2: Limpiando datos..."
$PYTHON $SCRIPTS_DIR/02_limpiar_datos.py --year $YEAR
if [ $? -ne 0 ]; then
    echo "❌ Error en limpieza. Pipeline detenido."
    exit 1
fi
...
echo ""
echo "⚙️  PASO 3: Creando features..."
$PYTHON $SCRIPTS_DIR/03_crear_features.py --year $YEAR
if [ $? -ne 0 ]; then
    echo "❌ Error en feature engineering. Pipeline detenido."
    exit 1
fi
...
if [ $? -ne 0 ]; then
    echo "❌ Error en generación de reporte. Pipeline detenido."
    exit 1
fi
...
log_step "🚀 Pipeline iniciado"

# Ejecutar pasos con logging
log_step "📥 Extrayendo datos..."
python scripts/01_extraer_datos.py --year 2024 2>&1 | tee -a $LOG_FILE
RESULT=${PIPESTATUS[0]}

if [ $RESULT -ne 0 ]; then
    log_step "❌ Pipeline falló en extracción"
    exit 1
fi

log_step "✅ Pipeline completado"
```

---

## 5.3 Orquestación con Python
...
- **Aristas**: dependencias (qué debe ejecutarse primero)
- **Acíclico**: no hay loops (A depende de B, B de C, C de A ❌)
...
1. **Paralelización**: tareas sin dependencias se ejecutan en paralelo
2. **Recuperación de errores**: re-ejecutar solo desde el punto de falla
3. **Claridad**: visualizar fácilmente el flujo
...
# Representación simple de un DAG
pipeline_dag = {
    ...
    while len(ejecutadas) < len(dag):
        for tarea, deps in dag.items():
            if tarea not in ejecutadas and puede_ejecutar(tarea):
                ...
                funciones[tarea]()  # Ejecutar función
                ejecutadas.add(tarea)
                print(f"✅ Completado: {tarea}\n")
```
...
**Apache Airflow** es la herramienta líder para orquestación de pipelines complejos.

### Características Principales

- 📅 **Scheduling**: ejecutar pipelines en horarios específicos
- 🔄 **Retries**: reintentos automáticos en caso de falla
- 📊 **Monitoring**: UI para visualizar estado de tareas
- 🔀 **Dependencias complejas**: DAGs sofisticados
- 🔌 **Integraciones**: conectores para AWS, GCP, bases de datos, etc.

### ¿Cuándo Usar Airflow?

- ✅ Pipelines complejos con muchas dependencias
- ✅ Necesidad de scheduling robusto
- ✅ Múltiples fuentes de datos
- ✅ Equipos grandes que necesitan monitoreo centralizado
- ❌ Proyectos pequeños (overhead innecesario)
- ❌ Pipelines que se ejecutan manualmente

### Recursos para Aprender

- 📚 Documentación oficial: [https://airflow.apache.org/docs/](https://airflow.apache.org/docs/)
- 🎓 Tutorial: [https://airflow.apache.org/docs/apache-airflow/stable/tutorial.html](https://airflow.apache.org/docs/apache-airflow/stable/tutorial.html)
- 📹 Episodio 140 de en_coders: [E140 - Airflow: Utilidad, elementos básicos y setup](https://www.youtube.com/watch?v=hh9P3dMOKwI&pp=ygURZW5fY29kZXJzIGFpcmZsb3c%3D)
- 📹 YouTube: Buscar "Airflow tutorial for beginners"
...
Los scripts completos están en los artifacts complementarios. Aquí un resumen de cada uno:
...
Crea visualizaciones y métricas resumidas.
...
1. **Definir parámetros al inicio** del script
2. **Usar nombres descriptivos** en MAYÚSCULAS para constantes
3. **Variables sensibles en .env**, nunca hardcodeadas
4. **argparse para scripts CLI** que necesitan flexibilidad
5. **Validar parámetros** antes de usarlos
6. Usar rutas relativas en lugar de absolutas. Si requieres usar rutas absolutas por algún motivo, nunca las hardcodees en el código. Úsalas como variables de ambiente o pásalas como argumentos.

## ✅ Estructura de Carpetas

1. **Separar datos crudos de procesados**
2. **Scripts numerados** para indicar orden de ejecución
3. **Código reutilizable en `src/`**
...
5. **Tests para validar** cada módulo

## ✅ Queries SQL

1. **Evitar SELECT *** excepto en exploración
2. **Parametrizar con precaución** (SQL injection)
3. **Comentar queries complejas**
4. **Usar CTEs** para legibilidad
5. **Índices en columnas** filtradas frecuentemente
...
Dado este código malo, refactorízalo aplicando lo aprendido:
...
Diseña la estructura de carpetas para un proyecto que:
- Extrae datos de 3 fuentes diferentes
- Tiene notebooks de exploración (notebooks de Quarto, no de Jupyer Notebook, ya que estos últimos causan muchos problemas de versionamiento y otros inconvenientes)
- Genera reportes diarios
- Tiene tests unitarios

## Ejercicio 4: Reporte Parametrizado

- Crea un .qmd que reciba parámetros y genere reportes personalizados según lo que el usuario necesite
...
- 📚 "Clean Code" - Robert C. Martin (aplicable a Python)
...
1. ✅ **Parametrizar código** para hacerlo flexible y mantenible
2. ✅ **Usar .env** para gestionar configuraciones sensibles
3. ✅ **Crear scripts CLI** con argparse
4. ✅ **Parametrizar queries SQL** de forma segura
5. ✅ **Organizar proyectos** con estructura de carpetas clara
6. ✅ **Modularizar código** en scripts especializados
7. ✅ **Orquestar pipelines** con Bash y Python
8. ✅ **Entender DAGs** y su rol en pipelines complejos

## Próximos Pasos

- 🎯 Aplicar parametrización en tus proyectos actuales
- 🎯 Crear tu template de proyecto con cookiecutter
- 🎯 Experimentar con pipelines simples
- 🎯 Investigar Airflow cuando tengas pipelines complejos
